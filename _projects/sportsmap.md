...
So, when the Lakers hit a three-pointer, Los Angeles flashes  **purple** **gold** **purple** **gold** **purple** **gold**, in real time
...
On a slow sports day, it is also an option to feed in random false scoring events, just to make the map dance and look pretty.  It is quite eye-catching.
...
The hardware challenge was both to create the map, and to wire and afix the LEDs properly to the map.
...
I used the same software that I used on my [UCLA Map]({{ site.baseurl  }}{% link _hobbies/UCLAmap.md %}), QGIS, the open source Geographic Information System.  Using open source data, I was able to create the following image, which cleanly fit the 3:2 ratio that I was aiming for.
...
Since the LEDs in the map had to show different colors at different cities simultaneously, it was necessary to use an addressable RGB LED strip, 
so I could control the LEDs from a single microcontroller, which in my case was an Arduino Uno. Since the line can be run from a single GPIO pin, using such a large device was not entirely necessary,
but since the electronics were all going to hide in the space behind the canvas, real estate was not a concern, so I just used the Uno I had on hand.

The real difficulty was that the cities on this map were obviously not condusive to being backlit by a linearly spaced LED strip, at least not without wasting a ton of LEDs as I wove from city to city behind the canvas.
...
Little did I know, the API was depreceated in 2014 and they weren't handing out more developers keys....
...
The Python script runs a Node script that outputs scoring events, which are piped over USB to an Arduino ready to recieve the messages and flash the LEDs.
